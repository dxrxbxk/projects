...
/*   Updated: 2022/11/21 04:58:09 by momadani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <minishell.h>

static int	ft_free_ast_node(t_ast **ast)
{
	if (!ast || !*ast)
		return (1);
	if ((*ast)->token)
...
static int	ft_free_cmd_args_branch(t_ast **ast)
{
	if (!ast || !*ast)
		return (0);
...
static int	ft_free_cmd_redir_branch(t_ast **ast)
{
	if (!ast || !*ast)
		return (0);
...
static int	ft_free_pipe_seq(t_ast **ast)
{
	if (!ast || !*ast)
		return (0);
...
int	ft_free_ast_child(t_ast **ast)
{
	...
	{
		ft_free_ast_child(&(*ast)->left);
		ft_free_ast_child(&(*ast)->right);
	}
	else if ((*ast)->token->type == CMD)
	{
		ft_free_cmd_redir_branch(&(*ast)->left);
